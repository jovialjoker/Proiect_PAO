...
public class CRUDMenu {
    ...
    private final SplitController _splitController;
    private final UserController _userController;

    private CRUDMenu()
    {
        _splitController = new SplitController();
        _userController = new UserController();
    }

    public static synchronized CRUDMenu getInstance()
    {
        if (menu == null)
            menu = new CRUDMenu();
                                ...
                                if (workoutList.size() != 0) {
                                    do {
                                        ...
                                        try {
                                            ...
                                            TreeMap<Integer, Workout> oldWorkouts = newSplit.getWorkouts();
                                            oldWorkouts.put(oldWorkouts.size(), workout);
                                ...
                                if (splits.size() != 0) {
                                    do {
                                        ...
                                        try {
                                            ...
                                            for (Map.Entry<Integer, Workout> workout: split.getWorkouts().entrySet()) {
                                                System.out.println((workout.getKey() + 1) + workout.getValue().getName());

                                                for(Exercise exercise: workout.getValue().getExercises()){
                                                    System.out.println("\t-" + exercise.getName());
                                                    if (exercise.getClass() == Bodybuilding_Exercise.class) {
                                                        System.out.println("\t\t" + ((Bodybuilding_Exercise) exercise).getRepsNo() + " reps;");
                                                        System.out.println("\t\t" + ((Bodybuilding_Exercise) exercise).getWeight() + " kgs;");
                                    ...
                                    System.out.println("\n There are no splits! Create some workouts and come back!");
                                }
                                break;
                            }
                            case "3": {
